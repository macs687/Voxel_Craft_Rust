...
fn load_png(filename: &str) -> Result<(Vec<u8>, i32, i32), image::error::ImageError>{
...
fn crate_texture_from_png(data: Vec<u8>, width: i32, height: i32) -> GLuint{
...
pub fn load_texture(filename: &str) -> Result<Texture, String> {
    let (data, width, height) = match load_png(filename) {
        ...
        Err(err) => {
            return Err(format!("ERROR::LOADING fail texture load: {}", err));
        }
    };

    let texture_id = crate_texture_from_png(data, width, height);
